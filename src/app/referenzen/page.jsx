    ...
    return (
        <div className="pt-32 pb-24 min-h-screen relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-4 relative z-10">
                <div className="ceramic-hero rounded-tile-2xl p-8 sm:p-14 text-center space-y-4 relative overflow-hidden">
            ...
            {PORTFOLIO_PROJECTS.length > 0 && (
                <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10" aria-labelledby="projekte-heading">
                    <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
                        <span className="eyebrow eyebrow-orange">Projektdokumentationen folgen</span>
                    ...
                    <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {PORTFOLIO_PROJECTS.map((project) => {
                            ...
                            return (
                                <li key={project.id}>
                                    <article className="group glass-surface p-5 rounded-tile-xl h-full flex flex-col hover:-translate-y-0.5 hover:border-orange-500/80 hover:shadow-[0_20px_40px_-12px_rgba(15,23,42,0.14)] transition-all duration-300">
                                        {/* Keramik-Panel statt Foto */}
                                        <div
                                            className="relative h-40 rounded-tile-lg border border-slate-200 bg-gradient-to-br from-orange-50/70 via-white to-amber-50/50 overflow-hidden flex items-center justify-center"
                                            style={TILE_PATTERN_STYLE}
                                            aria-hidden="true"
                                        >
                                            <div className="w-16 h-16 rounded-tile-md bg-white/90 border border-white/80 shadow-sm flex items-center justify-center">
                                        ...
                                        <div className="px-3 pt-5 space-y-3 flex-1">
                                            <div>
                                                <span className="text-xs font-black px-3 py-1 rounded-tile-pill bg-orange-50 text-orange-800 border border-orange-200">
                                                    {getCategoryName(project.category)}
                                                </span>
                                            </div>
                                            <p className="text-xs font-bold text-slate-700 flex items-start gap-1.5">
                                                <Camera className="w-3.5 h-3.5 text-orange-600 shrink-0 mt-px" aria-hidden="true" />
